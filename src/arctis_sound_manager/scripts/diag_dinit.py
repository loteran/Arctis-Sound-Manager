...
"""asm-diag-dinit — Read-only diagnostic for dinit-based ASM installations."""
        ...
        if svc == "pipewire-filter-chain" and content is not None:
            ...
            if cmd:
                ...
                if conf_path is not None:
                    ...
                    if is_absolute:
                        ok("filter-chain.conf path is absolute", conf_path)
                    else:
                        err("filter-chain.conf path is RELATIVE — must be absolute", conf_path)
                    conf_exists = Path(conf_path).exists() if is_absolute else False
                    result["filter_chain_conf_exists"] = conf_exists
                    if conf_exists:
                        ok("filter-chain.conf file exists", conf_path)
                    else:
                        (err if is_absolute else miss)(
                            "filter-chain.conf file does NOT exist", conf_path,
                        )
                else:
                    miss("could not extract -c <conf> from command line")

    # GUI autostart via XDG
    print("\n  ── arctis-gui (XDG autostart)")
    xdg_file = Path.home() / ".config" / "autostart" / "arctis-gui-autostart.desktop"
    xdg_exists = xdg_file.exists()
    result["xdg_autostart"] = xdg_exists
    (ok if xdg_exists else miss)(
        "XDG autostart entry for arctis-gui",
        str(xdg_file) if xdg_exists else "absent — GUI won't start on login",
    )
...
def check_pipewire() -> dict:
    ...
    # Config file candidates — check both user and system paths (Artix may differ)
...
def check_asm() -> dict:
    ...
    for binary in ("asm-daemon", "asm-gui", "asm-router", "asm-clipd",
...
def check_corrections(services: dict, pipewire: dict) -> dict:
    ...
    if fc_abs is True:
        ok("filter-chain.conf path is absolute")
    elif fc_abs is False:
        err("filter-chain.conf path is RELATIVE")
    else:
        miss("filter-chain.conf path (service file absent or command not parsed)")
    result["correction_b_absolute"] = fc_abs

    fc_exists = services.get("filter_chain_conf_exists")
    if fc_exists is True:
        ok("filter-chain.conf file exists at that path")
    elif fc_exists is False:
        err("filter-chain.conf file does NOT exist at that path")
    else:
        miss("filter-chain.conf file check skipped")
    result["correction_b_exists"] = fc_exists

    am_present = services["services_present"].get("arctis-manager", False)
    (ok if am_present else miss)("arctis-manager service file present")
    ...
    xdg_ok = services.get("xdg_autostart", False)
    (ok if xdg_ok else miss)(
        "XDG autostart for arctis-gui present",
        "~/.config/autostart/arctis-gui-autostart.desktop",
    )
    result["xdg_autostart"] = xdg_ok
...
def main() -> None:
    ...
    summary = {
        ...
        "xdg_autostart": r_svc.get("xdg_autostart", False),
