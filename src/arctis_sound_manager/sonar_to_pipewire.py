...
{links_text}
    ...
    # Issue #223: Media channel needs node.pause-on-idle = false so the
    # HeSuVi convolver stays warm across track changes (browser closes/reopens
    # stream). Without this, the convolver resets its state on resume → pop.
    _pause_on_idle_line = (
        '        node.pause-on-idle  = false\n'
    ) if channel == "media" else ''
...
def _ensure_media_pause_on_idle(path: Path) -> bool:
    """Add ``node.pause-on-idle = false`` to Media channel confs (issue #223).

    The Media channel (EQ + HeSuVi) needs the convolver to stay warm across
    stream gaps (browser track changes). Without this, the HeSuVi convolver
    resets its state on resume → audible pop/click.
    ...
    - The Media EQ conf (sonar-media-eq.conf) is NOT regenerated by a version
      bump alone — it only regenerates on user EQ edit. So existing installs
      need this in-place fix to carry the fix forward without flattening EQ.
    """
    # Only applies to Media channel confs
    media_confs = (
        "sonar-media-eq.conf",
        "sink-virtual-surround-7.1-hesuvi-media.conf",
    )
    if path.name not in media_confs:
        return False
...
def check_and_fix_stale_configs() -> tuple[bool, bool]:
    ...
    # ── Issue #223: Media channel needs node.pause-on-idle = false ─────────────
    # The Media HeSuVi convolver resets its state on stream resume → pop/click.
    # This in-place repair adds the property to existing Media confs without
    # regenerating (which would flatten EQ or lose saved spatial settings).
    for _media_conf_name in (
        "sonar-media-eq.conf",
        "sink-virtual-surround-7.1-hesuvi-media.conf",
    ):
        if _ensure_media_pause_on_idle(_CONF_DIR / _media_conf_name):
            log.warning(
                "%s had no node.pause-on-idle — patched in place so the "
                "convolver stays warm across track changes (issue #223)",
                _media_conf_name,
            )
            fixed = True
...
def generate_hesuvi_conf(
    ...
    # Issue #223: Media channel needs node.pause-on-idle = false so the HeSuVi
    # convolver stays warm across track changes (browser closes/reopens stream).
    # Without this, the convolver resets its state on resume → audible pop/click.
    _pause_on_idle_line = (
        '        node.pause-on-idle = false\n'
    ) if channel == "media" else ''
